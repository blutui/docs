---
title: 'Canopy Elements'
description: 'Learn how to use Canopy elements in Blutui'
---

<Callout type="warning">
  Canopy elements are deprecated and replaced by [Canopy Blocks](/docs/canopy/canopy-blocks), which let content
  editors manage whole sections instead of individual fields. Use block templates for new projects.
</Callout>
...
<Callout>
  Canopy elements are **page-specific** — meaning each element's content is tied to the page it appears on. If you need
  reusable, global content (like a footer or announcement banner), consider using a
  [Collection](/docs/collections/introduction) instead.
</Callout>
...
<Callout>
  The **handle** must be unique within each page and is used to identify the element in the Canopy editor. When you edit
  a page in the dashboard, these handles map to fields in the Canopy editor for that page.
</Callout>
