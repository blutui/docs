...
Every setting requires a `name` and a `type`. Most types also accept a `label`, a `default`, and a few type-specific attributes. See [Canopy Block Settings](/docs/canopy/canopy-block-settings) for the full list of setting types.
