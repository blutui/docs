...
Create a new Blutui project. In a single command, `create` scaffolds your chosen Canvas into a new directory, initializes Courier (generating the `courier.json` file), and can optionally install the [Blutui Context](/docs/getting-started/agentic-development) (MCP server and Skills) to improve AI assisted coding.
...
| `--context` | Install the Blutui Context (MCP server and Skills) without prompting. |
| `--client` | The name of the MCP client to configure when installing the Blutui Context (for example `claude-code`, `cursor`, `vscode`). |
...
# Scaffold a project with a Cassette and install the Blutui Context
