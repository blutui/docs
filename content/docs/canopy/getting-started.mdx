...
## Creating your first Canopy element

1. Open your project from the Blutui Agency Console.

2. Navigate to your project's **views** directory.

3. Open or create a layout or page template file.
...
Each element requires a **Handle** (unique identifier) and **Attributes** (configuration options).
...
<Callout>
The **Handle** must be unique within each page and is used to identify the element in the Canopy editor.
</Callout>
...
To edit the content for your Canopy elements, open the page in the dashboard using

```twig
{% include 'layouts/default.canvas' %}
```

This will render all the Canopy elements defined in your template.
