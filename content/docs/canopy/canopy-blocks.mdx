...
The `children` object accepts four optional sub-keys, documented in detail on the [Canopy Nested Blocks](/docs/canopy/canopy-nested-blocks#the-children-config) page:
...
Block areas also accept options to restrict the templates they accept (`allow`), cap the number of blocks (`limit`), and fix their structure (`locked`). See the [canopy.blocks](/canvas/functions/canopy/canopy-blocks) reference for all of them.
