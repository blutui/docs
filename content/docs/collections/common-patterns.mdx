---
title: 'Common Patterns for Collections'
...
Use `group_by` to organize items by a field:
...
When `allow_multiple_file_uploads: false`, the field returns a **string URL**:
...
When `allow_multiple_file_uploads: true`, the field returns an **array of URL strings**:
...
| ------------------------------------ | ------------- | ------------- | ---------------------------------------------------------- |
| `allow_multiple_file_uploads: false` | String URL    | Direct        | `{{ item.photo }}`                                         |
| `allow_multiple_file_uploads: true`  | Array of URLs | Loop or index | `{{ item.gallery[0] }}` or `{% for img in item.gallery %}` |
