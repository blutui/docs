...
You can also optimise an image in your project's `public` directory by passing its `asset` URL to the filter:

```canvas
{{ asset('images/header-background.png') | image_url({ format: 'webp' }) }}
```
