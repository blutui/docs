...
A form picked from the site's forms. It works like the [Collection](#collection) selector: the editor sees a dropdown of the site's forms and chooses one, so a single block template can render whichever form the page needs — a contact form here, a newsletter signup there — without hard-coding a handle.
